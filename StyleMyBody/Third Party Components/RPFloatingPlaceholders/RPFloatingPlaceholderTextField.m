...
- (void)hideFloatingLabel
{
    ...
    view.frame=CGRectMake(self.frame.origin.x, self.frame.size.height+self.frame.origin.y, self.frame.size.width, 2);
    view.backgroundColor=[UIColor redColor];
    [self.superview addSubview:view];
    
    UILabel *warningLbl=[[UILabel alloc]init];
    warningLbl.frame=CGRectMake(view.frame.origin.x, view.frame.size.height+view.frame.origin.y+3, view.frame.size.width, 10);
    warningLbl.text=@"  Please enter valid Email";
    [self.superview addSubview:warningLbl];
...
- (void)textFieldDidBeginEditing:(NSNotification *)notification
{
    ...
    [self.delegateObj receivedData:@"venky"];
}
